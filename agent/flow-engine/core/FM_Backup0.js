...
export function FlowManager({initialState, nodes, instanceId}={initialState:{}, nodes:[], instanceId: undefined}) {
   ...
   * It uses the 'scope' object (expected to be globally or lexically available).
   * The nodeIdentifier can be:
   * 1. A direct key in 'scope' for a user-added simple function.
   * 2. A fully qualified "id:name" string, which is a key in 'scope' for a NodeRegistry definition.
  ...
  function resolveNodeFromScope(nodeIdentifier) {
    ...
    // 1. Direct match for user-added simple functions (key is nodeIdentifier, value is the function)
    if (typeof scope[nodeIdentifier] === 'function') {
      return scope[nodeIdentifier];
    }

    // 2. Match for fully qualified "id:name" (key is nodeIdentifier, value is NodeDefinition)
    if (scope[nodeIdentifier] && typeof scope[nodeIdentifier].implementation === 'function') {
      return scope[nodeIdentifier].implementation;
    }

    // 3. Attempt to resolve by ID (e.g., "text.analysis.sentiment")
    //    It will look for keys like "text.analysis.sentiment:Some Name"
    for (const key in scope) {
      if (Object.prototype.hasOwnProperty.call(scope, key) && key.startsWith(nodeIdentifier + ":")) {
        if (scope[key] && typeof scope[key].implementation === 'function') {
          return scope[key].implementation; // Found by ID prefix
        }
      }
    }

    // 4. Attempt to resolve by Name (e.g., "Sentiment Analyzer")
    //    It will look for keys like "some.id:Sentiment Analyzer"
    //    This is less specific, so it's checked after ID.
    for (const key in scope) {
      if (Object.prototype.hasOwnProperty.call(scope, key) && key.endsWith(":" + nodeIdentifier)) {
        if (scope[key] && typeof scope[key].implementation === 'function') {
          return scope[key].implementation; // Found by Name suffix
        }
      }
    }
  ...
  async function evaluateNode(node) {
    ...
    if (typeof node === 'function') {
      ...
      if (implementation) {
        ...
        console.error(`[FlowManager:${flowInstanceId}] Node/Function '${node}' not found in scope or registry.`);
        output = { edges: ['error'], errorDetails: `Node/Function '${node}' not found` };
      }
