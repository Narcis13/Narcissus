...
Analyze the FlowManager.js file to deeply understand the intrinsic logic and implementation of this framework. 
YOUR TASK is to generate a text prompt i will use to ask AI models like you to generate a full node implementation following the blueprint of those 3 nodes already implemented as an example.
The user will provide a dranular , prefarable single purpose actionable task in a prompt variable {{task}} with some short description of what node should DO when integrated in a workflow using this framework .
IMPORTANT:
- insert an example implementation in the prompt you generate so the resulted node implementation will follow strictly the guidelines you will provide
- be aware to not omit something in the prompt  